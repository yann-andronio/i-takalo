...
export default function ProductSellForm() {
  ...
  const handleAddProduct: SubmitHandler<FormData> = async data => {
    ...
    if (selectedImages.length === 0) {
      Alert.alert(
        'Erreur',
        'Veuillez sélectionner au moins une image pour votre produit.',
      );
      return;
    }
    ...
    try {
      ...
      const newProduct = response.data as ProductDataI;
      addProduct(newProduct);

      Alert.alert('Succès', 'Votre produit a été mis en vente.', [
        {
          text: 'OK',
          onPress: () => {
            reset();
      ...
      console.error(
        'Erreur création produit (détails du serveur):',
      ...
      Alert.alert(
        ...
        error.response?.data?.message ||
          error.message ||
          'Impossible de créer le produit (Erreur Réseau ou Timeout)',
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      <View className="mb-5">
        ...
        {/* Prix */}
        <Text className="text-base font-bold mb-2 mt-5">
          Prix (en Ar)<Text className="text-red-500">*</Text>
        </Text>
        <Controller
          control={control}
          name="price"
